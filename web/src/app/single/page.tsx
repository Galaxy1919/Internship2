...
export default function Home() {
  return (
    <div>
      <section className="border-b pb-8 pt-6" style={{ borderColor: "var(--border)" }}>
        ...
        <p className="mt-4 max-w-2xl text-base leading-relaxed" style={{ color: "var(--text-dim)" }}>
      ...
      <section className="panel mt-10 grid gap-6 p-6 sm:grid-cols-3">
        {[
          ["01", "全部浏览器端计算", "不依赖后端服务，加密过程完全在本地可见、可审计"],
          ["02", "过程逐步可视化", "方阵、矩阵、轮函数、交换记录全部展开为可交互步骤"],
          ["03", "面向教学验证", "每个实验附带标准向量与对照说明，演示结果可手工验算"],
        ].map(([icon, title, desc]) => (
          <div key={title} className="flex gap-3">
            <div className="font-mono text-xs" style={{ color: "var(--accent)" }}>{icon}</div>
            <div>
